    ...
    def affine_transform(self, img, border):
        ...
        if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
            if img.ndim == 3 and img.shape[2] > 4:
                # OpenCV does not apply a 3-value border tuple consistently to >4-channel arrays.
                # Warp paired multi-modal images in channel blocks so geometric augmentation remains synchronized
                # while every modality receives the same padding value.
                chunks = []
                for start in range(0, img.shape[2], 3):
                    chunk = img[:, :, start : start + 3]
                    border_value = (114,) * chunk.shape[2]
                    if self.perspective:
                        chunks.append(cv2.warpPerspective(chunk, M, dsize=self.size, borderValue=border_value))
                    else:  # affine
                        chunks.append(cv2.warpAffine(chunk, M[:2], dsize=self.size, borderValue=border_value))
                img = np.concatenate([x[..., None] if x.ndim == 2 else x for x in chunks], axis=-1)

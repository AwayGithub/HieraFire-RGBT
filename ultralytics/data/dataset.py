...
class RandomHSV6Channel:
    """
    适配6通道图像的RandomHSV：仅对前3个通道（VIS_BGR）进行HSV与对比度增强
    """
    ...
    def __call__(self, labels):
        """Applies HSV and Contrast augmentation to the visible-light modality only."""
        img = labels["img"]
        ...
        if img.shape[2] == 6:
            ...
            # 根据 input_mode 决定是否增强 VIS_BGR 通道 (0-2)
...
class RectMosaic(Mosaic):
    """Four-image Mosaic with a native rectangular output canvas."""

    def __init__(self, dataset, imgsz, p=1.0):
        assert isinstance(imgsz, (list, tuple)) and len(imgsz) == 2
        super().__init__(dataset=dataset, imgsz=max(imgsz), p=p)
        self.height, self.width = map(int, imgsz)
        # RandomPerspective interprets this as (height, width) border offsets.
        self.border = (-self.height // 2, -self.width // 2)

    def _mosaic4(self, labels):
        """Build a four-image mosaic directly at the configured HxW target shape."""
        mosaic_labels = []
        yc = int(np.random.uniform(self.height // 2, self.height * 1.5))
        xc = int(np.random.uniform(self.width // 2, self.width * 1.5))
        for i in range(4):
            ...
            if i == 0:
                img4 = np.full((self.height * 2, self.width * 2, img.shape[2]), 114, dtype=np.uint8)
                x1a, y1a, x2a, y2a = max(xc - w, 0), max(yc - h, 0), xc, yc
                x1b, y1b, x2b, y2b = w - (x2a - x1a), h - (y2a - y1a), w, h
            elif i == 1:
                x1a, y1a, x2a, y2a = xc, max(yc - h, 0), min(xc + w, self.width * 2), yc
                x1b, y1b, x2b, y2b = 0, h - (y2a - y1a), min(w, x2a - x1a), h
            elif i == 2:
                x1a, y1a, x2a, y2a = max(xc - w, 0), yc, xc, min(self.height * 2, yc + h)
                x1b, y1b, x2b, y2b = w - (x2a - x1a), 0, w, min(y2a - y1a, h)
            else:
                x1a, y1a, x2a, y2a = xc, yc, min(xc + w, self.width * 2), min(self.height * 2, yc + h)
    ...
    def _cat_labels(self, mosaic_labels):
        """Concatenate labels and clip them to the rectangular mosaic canvas."""
        ...
        final_labels = {
            ...
            "resized_shape": (self.height * 2, self.width * 2),
            "cls": np.concatenate(cls, 0),
            "instances": Instances.concatenate(instances, axis=0),
            "mosaic_border": self.border,
        }
        final_labels["instances"].clip(self.width * 2, self.height * 2)
...
class FLAME2Dataset(BaseDataset):
    ...
    def get_image_and_label(self, index):
        ...
        # Current dual-stream YAMLs use raw VIS_BGR then IR_BGR. Format reverses all six channels to model IR_RGB then VIS_RGB.
        # Keep all six channels even in single-modality modes for visualization.
    ...
    def get_labels(self):
        ...
        # Include the first-modality directory so datasets with the same labels but different channel order do not collide.
        split = Path(self.img_path).stem if self.img_path else "flame2"
        prefix = f"{split}-{self.rgb_dir.name}"
    ...
    def _build_6channel_transforms(self, dataset, imgsz, hyp):
        """
        Build paired-safe default detection augmentations for 6-channel RGB-T inputs.

        Geometric and mixing transforms operate on the concatenated six channels,
        so paired RGB/IR pixels retain the same spatial transform. Photometric HSV
        perturbation is restricted to the visible-light channels by
        ``RandomHSV6Channel``.
        """
        from .augment import CopyPaste, MixUp, RandomFlip, RandomPerspective
        ...
        mosaic_cls = RectMosaic if isinstance(imgsz, (list, tuple)) else Mosaic
        mosaic = mosaic_cls(dataset, imgsz=imgsz, p=hyp.mosaic)
        affine = RandomPerspective(
            ...
            pre_transform=LetterBox(new_shape=new_shape),
        )
        # Mosaic is a mixing transform: when p=0, its nested affine would never
        # run on the original sample. Keep non-Mosaic geometric augmentation at
        # the top level so it is applied once and synchronously to all six
        # channels (VIS_BGR|IR_BGR) before VIS-only HSV and flips.
        if hyp.mosaic == 0.0:
            ...
            if hyp.copy_paste_mode == "flip":
                ...
                pre_transform.append(
                    CopyPaste(
                        dataset,
                        pre_transform=Compose([mosaic_cls(dataset, imgsz=imgsz, p=hyp.mosaic), affine]),
                        p=hyp.copy_paste,
...
class RGBT3MDataset(FLAME2Dataset):
    ...
    def __init__(self, *args, data=None, task="detect", **kwargs):
        """初始化RGBT3M数据集加载器"""
        image_ext = str(data.get("image_ext", ".jpg")).strip().lower()
    ...
    def get_img_files(self, img_path):
        ...
        if isinstance(img_path, (str, Path)) and str(img_path).endswith('.txt'):
            ...
            rgb_files = []
            for idx in indices:
                # 按照 RGBT-3M 结构拼接路径，例如 RGB/train/xxx.jpg 或 RGB/val/xxx.png。
                rgb_path = self.rgb_dir / subset / f"{idx}{self.image_ext}"
                if rgb_path.exists():
                    rgb_files.append(str(rgb_path))
                else:
